    ...
    if isinstance(m, (nn.Conv1d, nn.Linear)):
        if mode is not None:
            if mode == 'xavier_uniform':
                ...
                raise ValueError(
                    "Unknown Initialization mode: {0}".format(mode))
...
def _se_pool_step_script_infer(x: torch.Tensor, context_window: int, mask: torch.Tensor):
    ...
    if timesteps < context_window:
        y = torch.sum(x, dim=-1, keepdim=True) / \
            mask.sum(dim=-1, keepdim=True).to(x.dtype)
    else:
...
def _se_pool_step_script_train(x: torch.Tensor, context_window: int, mask: torch.Tensor):
    ...
    if timesteps < context_window:
        ...
        start_idx = torch.randint(
            0, timesteps - context_window, size=[1], dtype=torch.int32)[0]
        # [B, C, context_window]
        x = x[:, :, start_idx: (start_idx + context_window)]
        # [B, 1, context_window]
        mask = mask[:, :, start_idx: (start_idx + context_window)]
...
class MaskedConv1d(nn.Module):
    ...
    def __init__(
        ...
        self.same_padding = (self.conv.stride[0] == 1) and (
            2 * self.conv.padding[0] == self.conv.dilation[0] *
        ...
        if self.pad_layer is None:
            ...
            self.same_padding_asymmetric = (self.conv.stride[0] == 1) and (
                sum(self._padding) == self.conv.dilation[0] * (
                    self.conv.kernel_size[0] - 1)
            )
    ...
    def mask_input(self, x, lens):
        max_len = x.size(2)
        mask = self.lens[:max_len].unsqueeze(
            0).to(lens.device) < lens.unsqueeze(1)
...
class SqueezeExcite(nn.Module):
    def __init__(
        ...
        if PYTORCH_QUANTIZATION_AVAILABLE and quantize:
            self.fc = nn.Sequential(
                quant_nn.QuantLinear(channels, channels //
                                     reduction_ratio, bias=False),
                activation,
                quant_nn.QuantLinear(
                    channels // reduction_ratio, channels, bias=False),
            )
        elif not PYTORCH_QUANTIZATION_AVAILABLE and quantize:
            raise ImportError(
                "pytorch-quantization is not installed. Install from "
    ...
    def _se_pool_step(self, x, mask):
        ...
        if self.context_window < 0:
            ...
            y = torch.sum(x, dim=-1, keepdim=True) / \
                mask.sum(dim=-1, keepdim=True).type(x.dtype)
        else:
    ...
    def make_pad_mask(self, seq_lens, max_audio_length, device=None):
        ...
        mask = self.seq_range[:max_audio_length].expand(
            seq_lens.size(0), -1) < seq_lens.unsqueeze(-1)  # [B, T]; bool
    ...
    def __init__(
        ...
        if future_context < 0:
            padding_val = get_same_padding(
                kernel_size[0], stride[0], dilation[0])
        else:

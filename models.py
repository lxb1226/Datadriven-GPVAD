...
class ResnetBlock(nn.Module):
    def __init__(self, in_filters, out_filters, num_kernels1, num_kernels2):
        ...
        self.conv1 = nn.Conv2d(
            self.in_filters, self.out_filters, (self.num_kernels1[0], self.num_kernels2))
        ...
        self.shortcut = nn.Conv2d(
            self.in_filters, self.out_filters, (1, self.num_kernels2))
...
class Resnet1D(nn.Module):
    def __init__(self, inputdim=64, outputdim=2, **kwargs):
        ...
        self.n_cnn_filters = kwargs.get("n_cnn_filters", [32, 64, 128])
        self.n_cnn_kernels = kwargs.get("n_cnn_kernels", [8, 5, 3])
        self.n_fc_units = kwargs.get("n_fc_units", [2048, 2048])

        # Resnet Blocks
        self.block1 = ResnetBlock(
            1, self.n_cnn_filters[0], self.n_cnn_kernels, 16)
        self.block2 = ResnetBlock(
            self.n_cnn_filters[0], self.n_cnn_filters[1], self.n_cnn_kernels, 1)
        self.block3 = ResnetBlock(
            self.n_cnn_filters[1], self.n_cnn_filters[2], self.n_cnn_kernels, 1)
        self.block4 = ResnetBlock(
            self.n_cnn_filters[2], self.n_cnn_filters[2], self.n_cnn_kernels, 1)
        ...
        self.relu2 = nn.ReLU()
        self.fc3 = nn.Linear(self.n_fc_units[1], outputdim)
